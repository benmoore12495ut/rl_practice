...
# new_model = Algo_Model(explore=0,game_player=1) # explore, game player
new_model = ModelAlgo()
# new_model = Algo(explore=0.5,game_player=1)
old_model = Algo(explore=0.3,game_player=2)
...
def play_game():
    ...
    while game_still_playing:
        if board.game_over:
            ...
            break
        if player == 1:
            # player_ = new_model.game_player
            # best_move = new_model.select_best_move(board.board,player_,move_num)+1
            best_move = new_model.make_predictions(board.board)
            # print(f'Player 1 move: {best_move}')
            # print(board.board)
        else: # player = 2
            player_ = old_model.game_player
            best_move = old_model.select_best_move(board.board,player_,move_num)+1
            # print(f'Player 2 move: {best_move}')
            # print(board.board)

        board.play_move(player,best_move)
        move_num += 1
...
while curr_game <= num_games:
    # define agents
    # new_model = Algo(explore=0.5,game_player=1)
    # old_model = Algo(explore=0.5,game_player=2)
    # play the game
    if curr_game % 10 ==0:
        print()
...
#             best_move = algo_0.select_best_move(board.board,player_)+1
#         else: # player = 2
#             player_ = algo_4.game_player
#             best_move = algo_4.select_best_move(board.board,player_)+1
#
#         moves[player_] += 1
...
# last_10_games_start_time = time.time()
# while curr_game <= num_games:
#     a_0_explore = random.randint(0,4) / 10
#     a_4_explore = random.randint(0,4) / 10
