...
class Algo_Model():
    def __init__(self,explore=0,game_player=1):
    ...
    def make_predictions(self,board,positions):
        best_move = 1
    ...
    def is_consecutive_subsequence(self,small_list, big_list):
        if not small_list:
            return True
        small_index = 0
    ...
    def generate_sublists_with_points(self,x):
    ...
    def points_from_list(self,list_):
        rewards = {
            1:0
            ,2:0
        }
        for x in [1,2]:
            max_points = 0
            potential_sublists_with_points = self.generate_sublists_with_points(x)
            for k,v in potential_sublists_with_points.items():
                if self.is_consecutive_subsequence(v[0],list_):
                    # print(f'sublist: {v[0]}')
                    # print(f'points: {v[1]}')
                    if v[1] > max_points:
                        max_points = v[1]
    ...
    def find_rewards(self,board):
        ...
        for r in range(6):
            ...
            rewards = self.points_from_list(created_list)
    ...
    def select_best_move(self,board,algo_player):
        ...
        # play the position if there is only a single position
        if len(avail_positions) == 1:
            best_move = avail_positions[0]
            return best_move
        # if there is more than one avail position -- continue on
        ...
        if best_move_points == 0:
            ...
            if len(best_move_options) ==1:
                best_move = best_move_options[0]
            else: # user model output
                best_move_options_adj = []
                for b in best_move_options:
                    best_move_options_adj.append(b+1)
                best_move = self.make_predictions(board,best_move_options_adj)-1
                # print(f'Model returned best move: {best_move}')


        # =====================
        # leverage explore/ exploit tradeoff
        # =====================
        explore_selection = random.randint(1,10) / 10
        # print(f'explore_selection: {explore_selection}')
        # print(f'self.explore: {self.explore}')
        if explore_selection <= self.explore:
            if len(avail_positions) == 1:
                best_move_explore = avail_positions[0]
            else:
                try:
                    move_ind = random.randint(0,len(avail_positions)-1)
                    best_move_explore = avail_positions[move_ind]
                except:
                    print(f'avail_positions: {avail_positions}')

            # only explore if you can't win on the next move or they will not win on the next move
            if best_move_points < 900 and best_move_points > -900:
                best_move = best_move_explore

        # print(f'best move at end of model: {best_move}')
        return best_move
    ...
    def agent(self,observation, configuration):
        ...
        best_move = self.select_best_move(board,player)
